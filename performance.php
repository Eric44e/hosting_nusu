...
try {
    switch ($page) {
        case 'transactions':
            $data = $pdo->query("SELECT reference, type, category, amount, payment_method, created_at FROM transactions ORDER BY created_at DESC LIMIT 50")->fetchAll();
        ...
        case 'performance':
            $data = $pdo->query("
                SELECT 
                    ...
                    t.specialization,
                    t.rating,
                    COUNT(CASE WHEN tk.status='completed' THEN 1 END) as total_jobs,
                    COUNT(CASE WHEN tk.status='completed' THEN 1 END) as completed_tickets,
                    COALESCE(SUM(CASE WHEN tk.status='completed' THEN tk.service_cost ELSE 0 END), 0) as earnings
                FROM technicians t 
                JOIN staff s ON s.id=t.staff_id
                LEFT JOIN tickets tk ON tk.technician_id=t.id
                GROUP BY t.id
                ORDER BY t.rating DESC, completed_tickets DESC
            ")->fetchAll();
            $columns = ['Technician', 'Specialization', 'Rating', 'Completed Tickets', 'Closed Jobs', 'Earnings (FRW)'];
            break;
        case 'roles':
...
<title><?= $info[0] ?> — NUSU Management System</title>
          ...
          <tr>
            ...
            <td data-key="<?= htmlspecialchars($key) ?>">
              ...
              <?php elseif(str_contains(strtolower($key), 'status')): ?>
                <?= statusBadge($val) ?>
              <?php elseif(str_contains(strtolower($key), 'rating')): ?>
                <?php 
                $rating = (float)$val;
                $fullStars = floor($rating);
                $hasHalf = ($rating - $fullStars) >= 0.5;
                echo str_repeat('<i class="fas fa-star" style="color:#fbbf24"></i>', $fullStars);
                if($hasHalf) echo '<i class="fas fa-star-half-alt" style="color:#fbbf24"></i>';
                echo ' <span style="color:var(--muted);font-size:0.85rem">(' . number_format($rating, 1) . ')</span>';
                ?>
              <?php else: ?>
